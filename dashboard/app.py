...
if modelo_dados is None:
    st.error(f"Semana atual: {semana_atual}")
...
with col1:
    stl.criar_card_metrica(
        titulo="Total de Alunos Analisados", 
...
with col2:
    stl.criar_card_metrica(
        titulo=f"Reprovação Prevista ({taxa_reprovacao:.1f}%)", 
...
with col2:
    ...
    with col_a:
        st.metric("📗 Média Aprovados", f"{media_aprov:.2f}")
    with col_b: 
        st.metric("📕 Média Reprovados", f"{media_reprov:.2f}")
