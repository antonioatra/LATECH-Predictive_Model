...
def criar_grafico_pizza(aprovados, reprovados, semana):
    ...
    labels = [f'Aprovados ({aprovados})', f'Reprovados ({reprovados})']
    ...
    fig.update_layout(
        title=f"Predições - Semana {semana}",
        showlegend=True,
        height=400
    )
    return fig
...
def criar_histograma_medias(df_resultados):
    ...
    medias_aprovados = df_resultados[df_resultados['Predição'] == 'Aprovado']['Media_Atual']
    medias_reprovados = df_resultados[df_resultados['Predição'] == 'Reprovado']['Media_Atual']
    ...
    fig_hist.add_trace(go.Histogram(
        x=medias_aprovados, name=f'Aprovados (média: {media_aprovados_calc:.2f})',
        nbinsx=15, marker_color='#22c55e', opacity=0.7,
        marker_line_color='#16a34a', marker_line_width=1
    ))
    
    fig_hist.add_trace(go.Histogram(
        x=medias_reprovados, name=f'Reprovados (média: {media_reprovados_calc:.2f})',
    ...
    fig_hist.add_vline(
        ...
        annotation_text=f"Média Aprovados: {media_aprovados_calc:.2f}", annotation_position="top"
    )
    
    fig_hist.add_vline(
        x=media_reprovados_calc, line_dash="dash", line_color="#dc2626", line_width=2,
        annotation_text=f"Média Reprovados: {media_reprovados_calc:.2f}", annotation_position="bottom"
    )
    
    fig_hist.update_layout(
        barmode='overlay', xaxis_title="Média das Notas", yaxis_title="Quantidade de Alunos",
